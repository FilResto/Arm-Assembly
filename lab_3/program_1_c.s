...
.text
	dadd R2,R0,R0 ; per accedere a v[i] 								FDEMW 5      10
	daddi R8,R0,32													   ; FDEMW 1    
	dadd R9,R0,R0 ;temp reg											   ;  FDEMW 1
	daddi R10,R10,0#sarebbe i										   ;   FDEMW 1
	daddi R12, R10, 1													;   FDEMW 1
	l.d F20, zero(R0)												   ;     FDEMW 1
	
LOOP:
	beq R10, R8, END_LOOP                                                                  ; FDEMW 2 9
    nop																					   ;  FDEMW 1
    ld R1, m(R0)                                                                           ;   FDEMW 1
    andi R4,R10, 1                                                                         ;    FDEMW 1
    beq R4,R0, FIRST_IS_EVEN                                                                   ; FDEMW 2
    l.d F2, v1(R2)                                                                         ;      FDEMW 1
    j FIRST_IS_ODD  																;	           FDEMW 1
...
FIRST_IS_EVEN:
	...
	j CONTINUE1																				;					  FDEMW	1
	l.d F9,v2(R2)#controllare																			;		   FDEMW 1


FIRST_IS_ODD:	
	;p= v1[i] / ((double)m* i))																;
	#l.d F2, v1(R2)																			;FDEMW	   28
...
CONTINUE1:
	...
	s.d F17,v7(R2)             																					;	FDEMW 0                     																						;  FDEMW 0
	j SECOND_CHECK

SECOND_CHECK:
	beq R12, R8, END_LOOP                                                                  ; FDEMW 2 9
    nop																					   ;  FDEMW 1
    ld R1, m(R0)                                                                           ;   FDEMW 1
    andi R4,R12, 1                                                                         ;    FDEMW 1
    beq R4,R0, SECOND_IS_EVEN                                                                   ;FDEMW 2
    l.d F2, v1+8(R2)                                                                         ;    FDEMW 1
    j SECOND_IS_ODD																			;      FDEMW 1
	
SECOND_IS_EVEN:
	#p= v1[i] * ((double)( m<< i)) /*logic shift */
	#l.d F2, v1(R2)																			;     FDEMW	1    17         
	dsllv R9,R1,R12 #bit shifting ledt su R1 o m di R2 o i e storing in R9 result			;	   FDEMW	1
	mtc1 R9,F3																				;	    FDEMW	1
	cvt.d.l F3,F3																			;		 FDEMW	1
	...
	j CONTINUE2																				;					  FDEMW	1
	l.d F9,v2+8(R2)#controllare																			;					   FDEMW 1


SECOND_IS_ODD:	
	;p= v1[i] / ((double)m* i))																;
	#l.d F2, v1(R2)																			;FDEMW	1    28
	...
	cvt.l.d F6,F3 ;tronca F3 e lo mette in F6												;	    FDEMW	0
    mfc1 R9,F6																				;	     FDEMW	0
	beq R9,R0, HANDLE_ZERO2																	;		  FDEMW	0
...
CONTINUE2:
	# Load operations first
	#l.d F9,v2(R2)       																;    
	l.d F10,p(R0)              															; FDEMW 1   40
	l.d F12,v3+8(R2)             															;  FDEMW 1
	l.d F14,v1+8(R2)             															;   FDEMW 1
	# Compute for v5[i]
	mul.d F11,F10,F9           # p * v2[i]												;    FDXXXXXXXXMW 8
	add.d F11,F11,F12          # (p * v2[i]) + v3[i]									;	  FDsssssssAAAMW 3
	l.d F12,v4+8(r2)             # Load v4[i] here to allow time for previous result		;	   FsssssssDEMW 0
	add.d F13,F11,F12          # ((p * v2[i]) + v3[i]) + v4[i]							;	           FDXXXMW 3
	...
	# Store results																		;						  
	s.d F13,v5+8(R2)             																				;     FDEMW 0
	s.d F15,v6+8(R2)             																					;  FDEMW 0
	s.d F17,v7+8(R2)             																					;	FDEMW 0

	# Pointer adjustments for next iteration
	daddi R2,R2,16             																					;	 FDEMW 0
