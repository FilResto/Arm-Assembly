...
Reset_Handler   PROC
				...
				;SVC		0x10	;0x000000DA
				mov r4, #7
				SVC			0x04 ;RESET R0, 0x01 RESET R1 e così via
				SVC			0x0A ;NOP
				SVC			0x42 ;MEMCPY
...
SVC_Handler     PROC
				...
				MRS	R1, psp	;devo recuperare il valore del pc che è nell altro stack (process stack pointer)
				LDR R0, [R1, #24]	;0x000000DC ;faccio la cosa delle slide per andare a rpendere il valore del pc
				LDR R0, [R0,#-4]	;0x000000D8
				BIC R0, #0xFF000000
				LSR R0, #16 ;mi prende il valore della chiamata a svc, quindi se faccio svc 0x04 in r0 ci sarà 4
				; your code here
				;cmp r0, 0x10;questo check dovra essere fatto con diversi valori nell esercitazione, perche l importante è capire da dove si arriva per sapere cosa fare
				;bne uscita
				;nop
				CMP R0, #7 ;se minore uguale allora siamo nel reset
				BLE Handle_Reset
				;CMP R0,#15 ;vede se è minore di 15, forse va tolta perche non fa niente 
...
Handle_Reset
				...
				CMP     R0, #0
				MOVEQ   R0, #0            ; se r0 era nella chiamata allora verrà resettato
				CMP     R0, #1
				MOVEQ   R1, #0            
				CMP     R0, #2
				MOVEQ   R2, #0            
